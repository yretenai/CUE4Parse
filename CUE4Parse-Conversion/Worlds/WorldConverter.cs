...
public static class WorldConverter {
    public static (List<WorldActor> Actors, List<(int ActorId, int MaterialIndex, string MaterialName)>
        ...
        foreach (var actorIndex in level.Actors) {
            ...
            if (actorObject.Class is not UBlueprintGeneratedClass) continue;
    ...
    private static int CreateActor(UObject? component, string? name, List<string> actorIds, List<WorldActor> actors,
        ...
        if (parent is { IsNull: false }) {
            ...
            if (actor.Parent == -1) {
                actor.Parent = CreateActor(parent.ResolvedObject?.Load(), parent.ResolvedObject?.Name.Text, actorIds, actors, overrideMaterials);
            }
        ...
        var actorMaterials = component.GetOrDefault("OverrideMaterials", Array.Empty<FPackageIndex?>());
