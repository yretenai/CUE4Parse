...
public static class WorldConverter
{
        ...
        OverrideMaterials, List<WorldLandscape> Landscapes) ConvertWorld(UWorld world, ETexturePlatform platform)
    {
        ...
        if (!world.PersistentLevel.TryLoad<ULevel>(out var level)) return (actors, overrideMaterials, landscapes);
        ...
        ETexturePlatform platform)
    {
        ...
        {
            if (landscapeComponent == null) continue;
            ...
            if (heightmap == null || heightmap.IsNull) continue;

            var outerMost = heightmap.ResolvedObject;
            if (outerMost == null) continue;

            while (outerMost.Outer != null) outerMost = outerMost.Outer;
            ...
            for (var index = 0; index < weightmaps.Length; index++)
            {
                ...
                if (weightmap == null || weightmap.IsNull) continue;
        ...
        List<(int, int, string)> overrideMaterials, bool strict = false)
    {
        ...
        if (actorIds.Contains(componentName)) return actorIds.IndexOf(componentName);
        ...
        if (parent != null && !parent.IsNull)
        {
            ...
            if (actor.Parent == -1) return -1;
        }

        if (component.GetOrDefault<bool>("bHidden")) actor.Flags |= WorldActorFlags.Hidden;
        if (!component.GetOrDefault("bVisible", true)) actor.Flags |= WorldActorFlags.Hidden;

        if (!component.GetOrDefault("CastShadow", true)) actor.Flags |= WorldActorFlags.NoCastShadow;
        ...
        if (actorMaterials != null)
            for (var materialIndex = 0; materialIndex < actorMaterials.Count; materialIndex++)
            {
                ...
                if (actorMaterial == null) continue;
